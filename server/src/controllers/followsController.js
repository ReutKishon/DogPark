...
const follow = (req, res) => {
  ...
  const sql = `INSERT INTO follows (owner_id, dog_id) VALUES (?, ?) ON DUPLICATE KEY UPDATE created_at = CURRENT_TIMESTAMP`;

  req.db.query(sql, [userId, dogId], (err, result) => {
    if (err) {
...
const unfollow = (req, res) => {
  const { userId, dogId } = req.params;
  const sql = "DELETE FROM follows WHERE owner_id = ? AND dog_id = ?";
  req.db.query(sql, [userId, dogId], (err, result) => {
    if (err) {
...
const getFollowings = (req, res) => {
  ...
  console.log("here1");
  console.log("here2");
  const sql = "SELECT * FROM follows WHERE owner_id = ?";
